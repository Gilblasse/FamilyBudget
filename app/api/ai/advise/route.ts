...
import { adviseResponseSchema } from '@/lib/ai/schemas';
...
const requestSchema = z.object({
  snapshot: z.custom<BudgetSnapshot>(
...
Given a budget snapshot for a single pay period, identify which bills should be delayed, reduced, skipped, or paid partially so the household stays solvent and prioritizes essentials.
...
- "savingsAmount" is the dollar reduction in this period if the recommendation is applied (full bill amount for skip/delay, half for partial, the user's likely reduction for reduce).
...
export async function POST(req: Request) {
  ...
  const prompt = `Budget snapshot for the active period:\n\n${contextAsPromptJson(ctx)}`;
